...
async def test_get_users(client: AsyncClient):
    ...
    assert response.status_code == 200
    assert len(response.json()) == 1; expected_keys = {"id", "name", "email", "password_hash"}
...
async def test_get_user(client: AsyncClient):
    ...
    assert response.status_code == 200; expected_keys = {"id", "name", "email", "password_hash"}
    assert set(response.json().keys()) == expected_keys
    assert response.json()["name"] == "Nikola"
    assert response.json()["email"] == "user@example.com"
...
async def test_delete_user(client: AsyncClient):
    data = {"name": "Nikola", "email": "user@example.com", "password": "123"}
    data2 = {"name": "Bells", "email": "user1@example.com", "password": "123"}
    login_data = {"email": "user@example.com", "password": "123"}
    await client.post("/api/v1/register", json=data)
    ...
    assert get_response_2.status_code == 200; assert len(get_response_2.json()) == 1
...
async def test_refresh_user(client: AsyncClient):
    ...
    refresh_data = {"refresh_token": my_refresh_token}
...
async def test_put_user(client: AsyncClient):
    data = {"name": "Nikola", "email": "user@example.com", "password": "123"}
    data_2 = {"name": "Nikolay", "email": "usersss@example.com", "password": "1234"}
    login_data = {"email": "user@example.com", "password": "123"}
    reg_response = await client.post("/api/v1/register", json=data)
    user_id = reg_response.json()["id"]
    response = await client.post("/api/v1/login", json=login_data)
