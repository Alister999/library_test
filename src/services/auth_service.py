...
from src.core.database import UserRepository
...
async def reg_user(user: UserCreate, db: AsyncSession) -> UserResponse:
    ...
    if await repo.get_one_or_none(User.name == user.name):
        ...
        raise HTTPException(status_code=403, detail=f"User with username '{user.name}' already exist")
    if await repo.get_one_or_none(User.email == user.email):
        logger.warning(f"User with email '{user.email}' already exist")
        raise HTTPException(status_code=403, detail=f"User with email '{user.email}' already exist")
    new_user = User(name=user.name, email=user.email, password_hash=hash_password(user.password))
    logger.info("Add user to repo"); await repo.add(new_user)
    await db.commit()
...
async def login_user(user: UserLogin, db: AsyncSession) -> dict:
    ...
    repo = UserRepository(session=db)
    db_user = await repo.get_one_or_none(User.email == user.email)
...
async def get_this_user(user_id: int, db: AsyncSession) -> UserResponse:
    ...
    if not getting_user:
        ...
        raise HTTPException(status_code=404, detail=f"User with id '{user_id}' is absent")
...
async def change_this_user(data: UserCreate, user_id: int, db: AsyncSession) -> UserResponse:
    ...
    if updated_data["name"]:
        ...
        if getting_name_user:
            ...
            raise HTTPException(status_code=400, detail=f"User with name '{updated_data["name"]}' already exist")
        else:
    ...
    if updated_data["email"]:
        ...
        if getting_name_email:
            ...
            raise HTTPException(status_code=400, detail=f"User with email '{updated_data["email"]}' already exist")
        else:
